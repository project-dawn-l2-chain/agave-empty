...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("treasury_pubkey")
                .long("treasury-pubkey")
                .value_name("PUBKEY")
                .takes_value(true)
                .validator(is_pubkey)
                .help("ProjectDawn treasury pubkey. If not provided, uses the default placeholder."),
        )
        .get_matches();
    ...
    // ProjectDawn: Add treasury account to genesis (rent-exempt, system-owned).
    {
        let treasury_pubkey = pubkey_of(&matches, "treasury_pubkey")
            .unwrap_or(solana_runtime::projectdawn_config::DEFAULT_TREASURY_PUBKEY);
        let treasury_lamports = rent.minimum_balance(0);
        genesis_config.add_account(
            treasury_pubkey,
            AccountSharedData::new(treasury_lamports, 0, &system_program::id()),
        );
    }
