...
pub fn parse_stake(
    ...
    match stake_instruction {
        ...
        StakeInstruction::PermanentLock => {
            check_num_stake_accounts(&instruction.accounts, 3)?;
            Ok(ParsedInstructionEnum {
                instruction_type: "permanentLock".to_string(),
                info: json!({
                    "stakeAccount": account_keys[instruction.accounts[0] as usize].to_string(),
                    "clockSysvar": account_keys[instruction.accounts[1] as usize].to_string(),
                    "stakeAuthority": account_keys[instruction.accounts[2] as usize].to_string(),
        ...
        StakeInstruction::PassiveLock(tier) => {
            check_num_stake_accounts(&instruction.accounts, 2)?;
            Ok(ParsedInstructionEnum {
                instruction_type: "passiveLock".to_string(),
                info: json!({
                    "stakeAccount": account_keys[instruction.accounts[0] as usize].to_string(),
                    "stakeAuthority": account_keys[instruction.accounts[1] as usize].to_string(),
                    "tier": tier,
                }),
            })
        }
        StakeInstruction::EarlyUnlock => {
            check_num_stake_accounts(&instruction.accounts, 3)?;
            Ok(ParsedInstructionEnum {
                instruction_type: "earlyUnlock".to_string(),
                info: json!({
                    "stakeAccount": account_keys[instruction.accounts[0] as usize].to_string(),
                    "stakeAuthority": account_keys[instruction.accounts[1] as usize].to_string(),
                    "treasuryAccount": account_keys[instruction.accounts[2] as usize].to_string(),
                }),
            })
        }
        StakeInstruction::GovernanceUnlock => {
            check_num_stake_accounts(&instruction.accounts, 2)?;
            Ok(ParsedInstructionEnum {
                instruction_type: "governanceUnlock".to_string(),
                info: json!({
                    "stakeAccount": account_keys[instruction.accounts[0] as usize].to_string(),
                    "governanceAuthority": account_keys[instruction.accounts[1] as usize].to_string(),
                }),
            })
        }
    }
