...
pub enum StakeInstruction {
    ...
    ///
    /// The principal can never be withdrawn. Only excess lamports (rewards) may
    /// be withdrawn by the withdrawer. The account earns a 120% reward multiplier.
    /// Both staker and withdrawer must sign.
    ///
    /// # Account references
    ///   0. `[WRITE]` Delegated stake account
    ///   1. `[SIGNER]` Staker authority
    ///   2. `[SIGNER]` Withdrawer authority
    PermanentLock,

    /// Lock tokens in passive (non-delegated) staking with a tier (ProjectDawn).
    ///
    /// The account must be in `Initialized` state. Creates a passive stake with
    /// `voter_pubkey = Pubkey::default()` as sentinel. The `u8` is the tier (1-5).
    ///
    /// # Account references
    ///   0. `[WRITE]` Initialized stake account
    ///   1. `[]` Clock sysvar
    ...
    ///
    /// Calculates a penalty based on tier, deducts it, and sends it to the
    /// treasury. Resets the account to `Initialized` state.
    ///
    /// # Account references
    ///   0. `[WRITE]` Passive stake account (with tier > 0 and active lockup)
    ///   1. `[WRITE]` Treasury account to receive penalty
    ///   2. `[]` Clock sysvar
    ///   3. `[SIGNER]` Staker authority
    EarlyUnlock,

    /// Governance unlock (ProjectDawn — stub, disabled at launch).
    ///
    /// Reserved for future governance-controlled unlock of permanently locked stake.
...
#[cfg(feature = "bincode")]
pub fn permanent_lock(stake_pubkey: &Pubkey, staker: &Pubkey, withdrawer: &Pubkey) -> Instruction {
    let account_metas = vec![
        AccountMeta::new(*stake_pubkey, false),
        AccountMeta::new_readonly(CLOCK_ID, false),
    ...
    Instruction::new_with_bincode(ID, &StakeInstruction::PermanentLock, account_metas)
}

#[cfg(feature = "bincode")]
pub fn passive_lock(stake_pubkey: &Pubkey, staker: &Pubkey, tier: u8) -> Instruction {
    let account_metas = vec![
        AccountMeta::new(*stake_pubkey, false),
        AccountMeta::new_readonly(CLOCK_ID, false),
        AccountMeta::new_readonly(*staker, true),
    ];
    Instruction::new_with_bincode(ID, &StakeInstruction::PassiveLock(tier), account_metas)
}
...
pub fn early_unlock(
    ...
    Instruction::new_with_bincode(ID, &StakeInstruction::EarlyUnlock, account_metas)
}
