...
pub enum StakeError {
    ...
    // 17 — ProjectDawn errors
    /// Stake account is permanently locked and cannot be deactivated.
    PermanentlyLocked,

    /// Invalid passive staking tier (must be 1-5).
    InvalidPassiveTier,

    /// Account is not a passive stake or lockup is not active.
    NotPassiveStake,
}
...
impl FromPrimitive for StakeError {
    ...
    fn from_i64(n: i64) -> Option<Self> {
        ...
        } else if n == Self::PermanentlyLocked as i64 {
            Some(Self::PermanentlyLocked)
        } else if n == Self::InvalidPassiveTier as i64 {
            Some(Self::InvalidPassiveTier)
        } else if n == Self::NotPassiveStake as i64 {
            Some(Self::NotPassiveStake)
        } else {
            None
        }
    }
...
impl ToPrimitive for StakeError {
    ...
    fn to_i64(&self) -> Option<i64> {
        Some(match *self {
            ...
            Self::PermanentlyLocked => Self::PermanentlyLocked as i64,
            Self::InvalidPassiveTier => Self::InvalidPassiveTier as i64,
            Self::NotPassiveStake => Self::NotPassiveStake as i64,
        })
    }
    #[inline]
...
impl core::fmt::Display for StakeError {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        match self {
            ...
            StakeError::PermanentlyLocked => {
                f.write_str("stake account is permanently locked and cannot be deactivated")
            }
            StakeError::InvalidPassiveTier => {
                f.write_str("invalid passive staking tier (must be 1-5)")
            }
            StakeError::NotPassiveStake => {
                f.write_str("account is not a passive stake or lockup is not active")
            }
        }
    }
}
