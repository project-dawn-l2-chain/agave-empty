use {
    crate::stake_state::{
        authorize, authorize_with_seed, deactivate, deactivate_delinquent, delegate, early_unlock,
        initialize, merge, move_lamports, move_stake, new_warmup_cooldown_rate_epoch,
        passive_lock, permanent_lock, set_lockup, split, withdraw,
    },
    log::*,
...
declare_process_instruction!(Entrypoint, DEFAULT_COMPUTE_UNITS, |invoke_context| {
    ...
    match stake_instruction {
        ...
        StakeInstruction::PermanentLock => {
            ...
            let clock =
                get_sysvar_with_account_check::clock(invoke_context, instruction_context, 1)?;
            permanent_lock(&mut me, &clock, &signers)
        }
        StakeInstruction::PassiveLock(tier) => {
            let mut me = get_stake_account()?;
            let clock =
                get_sysvar_with_account_check::clock(invoke_context, instruction_context, 1)?;
            passive_lock(&mut me, &clock, tier, &signers)
        }
        ...
        StakeInstruction::GovernanceUnlock => {
            // Stub — disabled at launch. Returns error unconditionally.
            Err(InstructionError::InvalidInstructionData)
        }
    }
});
