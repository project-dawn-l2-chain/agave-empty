...
pub fn deactivate(
    ...
    if let StakeStateV2::Stake(meta, mut stake, stake_flags) = stake_account.get_state()? {
        meta.authorized.check(signers, StakeAuthorize::Staker)?;
        // ProjectDawn: Block deactivation of permanently locked accounts
        if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
            return Err(StakeError::PermanentlyLocked.into());
        }
...
/// Permanently lock a delegated stake account (ProjectDawn).
///
/// Requires both staker AND withdrawer signatures. The account must be
/// actively delegated (in Stake state, not deactivated). Sets the
/// PERMANENTLY_LOCKED bit in StakeFlags. Once locked, the principal can
/// never be withdrawn — only excess lamports (rewards) above the staked
/// amount + rent reserve can be withdrawn.
pub fn permanent_lock(
    stake_account: &mut BorrowedAccount,
    _clock: &Clock,
    signers: &HashSet<Pubkey>,
) -> Result<(), InstructionError> {
    if let StakeStateV2::Stake(meta, stake, mut stake_flags) = stake_account.get_state()? {
        // Require both staker and withdrawer signatures
        meta.authorized.check(signers, StakeAuthorize::Staker)?;
        meta.authorized.check(signers, StakeAuthorize::Withdrawer)?;

        // Must be actively delegated (not deactivated)
        if stake.delegation.deactivation_epoch != u64::MAX {
            return Err(StakeError::AlreadyDeactivated.into());
        }

        // Must not already be permanently locked
        if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
            return Err(StakeError::PermanentlyLocked.into());
        }

        // Must not be a passive stake (sentinel voter)
        if stake.delegation.voter_pubkey == Pubkey::default() {
            return Err(InstructionError::InvalidAccountData);
        }

        stake_flags.set(StakeFlags::PERMANENTLY_LOCKED);
        stake_account.set_state(&StakeStateV2::Stake(meta, stake, stake_flags))
    } else {
        Err(InstructionError::InvalidAccountData)
    }
...
/// Lock tokens in passive (non-delegated) staking with a tier (ProjectDawn).
///
/// The account must be in `Initialized(Meta)` state. Creates a `Stake` with
/// `voter_pubkey = Pubkey::default()` as sentinel for passive stake.
/// Sets lockup based on tier and encodes tier in StakeFlags bits 4-6.
pub fn passive_lock(
    stake_account: &mut BorrowedAccount,
    clock: &Clock,
    tier: u8,
    ...
    if tier < 1 || tier > 5 {
        return Err(StakeError::InvalidPassiveTier.into());
    }

    if let StakeStateV2::Initialized(mut meta) = stake_account.get_state()? {
        meta.authorized.check(signers, StakeAuthorize::Staker)?;

        let lamports = stake_account.get_lamports();
        let stake_amount = lamports.saturating_sub(meta.rent_exempt_reserve);
        if stake_amount == 0 {
            return Err(InstructionError::InsufficientFunds);
        }

        // Tier lockup epochs: [0, 0, 14, 36, 72, 144]
        let lockup_epochs: [u64; 6] = [0, 0, 14, 36, 72, 144];
        let lockup_duration = lockup_epochs[tier as usize];

        // Set lockup epoch
        if lockup_duration > 0 {
            meta.lockup.epoch = clock.epoch.saturating_add(lockup_duration);
        }

        // Create passive delegation with sentinel voter_pubkey
        #[allow(deprecated)]
        let delegation = Delegation {
            voter_pubkey: Pubkey::default(), // sentinel for passive stake
            stake: stake_amount,
            activation_epoch: clock.epoch,
            deactivation_epoch: u64::MAX,
            warmup_cooldown_rate: 0.0, // not used for passive
        };
        ...
        let mut stake_flags = StakeFlags::empty();
        stake_flags.set_passive_tier(tier);
...
/// Early unlock a passive stake with penalty (ProjectDawn).
///
/// Calculates penalty based on tier, deducts from stake account,
/// credits to treasury, and resets to Initialized state.
pub fn early_unlock(
    transaction_context: &TransactionContext,
    instruction_context: &InstructionContext,
    stake_account_index: IndexOfAccount,
    treasury_index: IndexOfAccount,
    _clock: &Clock,
    signers: &HashSet<Pubkey>,
) -> Result<(), InstructionError> {
    let mut stake_account = instruction_context
    ...
    if let StakeStateV2::Stake(meta, stake, stake_flags) = stake_account.get_state()? {
        meta.authorized.check(signers, StakeAuthorize::Staker)?;

        let tier = stake_flags.passive_tier();
        if tier == 0 || stake.delegation.voter_pubkey != Pubkey::default() {
            return Err(StakeError::NotPassiveStake.into());
        }

        // Passive tier effective APY basis points (out of 10_000):
        // tier 1: 25bp (0.25%), tier 2: 50bp, tier 3: 100bp, tier 4: 150bp, tier 5: 250bp
        let tier_rate_bp: [u64; 6] = [0, 25, 50, 100, 150, 250];
        let rate_bp = tier_rate_bp[tier as usize];

        // Penalty = 5 * effective_APY * staked_amount
        // = 5 * (rate_bp / 10_000) * stake = stake * rate_bp * 5 / 10_000
        let penalty = ((stake.delegation.stake as u128) * (rate_bp as u128) * 5 / 10_000) as u64;

        if penalty >= stake_account.get_lamports().saturating_sub(meta.rent_exempt_reserve) {
            return Err(InstructionError::InsufficientFunds);
        }

        // Deduct penalty from stake account
        stake_account.checked_sub_lamports(penalty)?;

        // Reset to Initialized state with cleared lockup
        let mut new_meta = meta;
        new_meta.lockup = Lockup::default();
        stake_account.set_state(&StakeStateV2::Initialized(new_meta))?;
        drop(stake_account);

        // Credit penalty to treasury
        let mut treasury = instruction_context
            .try_borrow_instruction_account(transaction_context, treasury_index)?;
        treasury.checked_add_lamports(penalty)?;
...
pub fn split(
    ...
    match stake_state {
        StakeStateV2::Stake(meta, mut stake, stake_flags) => {
            meta.authorized.check(signers, StakeAuthorize::Staker)?;
            // ProjectDawn: Block splitting permanently locked accounts
            if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
                return Err(StakeError::PermanentlyLocked.into());
            }
...
pub fn move_stake(
    ...
    // ProjectDawn: Block moving stake from permanently locked accounts
    if let StakeStateV2::Stake(_, _, source_flags) = source_account.get_state()? {
        if source_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
            return Err(StakeError::PermanentlyLocked.into());
        }
    }
...
pub fn move_lamports(
    ...
    // ProjectDawn: Block moving lamports from permanently locked accounts
    if let StakeStateV2::Stake(_, _, source_flags) = source_account.get_state()? {
        if source_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
            return Err(StakeError::PermanentlyLocked.into());
        }
...
pub fn withdraw(
    ...
    let (lockup, reserve, is_staked) = match stake_account.get_state()? {
        StakeStateV2::Stake(meta, stake, stake_flags) => {
            ...
            // ProjectDawn: Permanently locked accounts can only withdraw excess (rewards)
            if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
                let staked_and_reserve = checked_add(stake.delegation.stake, meta.rent_exempt_reserve)?;
                (meta.lockup, staked_and_reserve, true) // always considered staked
            } else {
...
pub(crate) fn deactivate_delinquent(
    ...
    if let StakeStateV2::Stake(meta, mut stake, stake_flags) = stake_account.get_state()? {
        ...
        // ProjectDawn: Block deactivation of permanently locked accounts via delinquent path
        if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
            return Err(StakeError::PermanentlyLocked.into());
        }
...
impl MergeKind {
    ...
    fn get_if_mergeable(
        ...
        match stake_state {
            StakeStateV2::Stake(meta, stake, stake_flags) => {
                // ProjectDawn: Block merging permanently locked accounts
                if stake_flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
                    return Err(StakeError::PermanentlyLocked.into());
                }
...
mod tests {
    ...
    // ── ProjectDawn PERMANENTLY_LOCKED guard tests ────────────────────

    #[test]
    fn test_split_permanently_locked_rejected() {
        let transaction_accounts = vec![(
            epoch_schedule::id(),
            create_account_shared_data_for_test(&EpochSchedule::default()),
        )];
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(StakeStateV2::size_of());
        let stake_lamports = rent_exempt_reserve + 1_000_000;
        let meta = Meta {
            rent_exempt_reserve,
            ..Meta::auto(&authority_pubkey)
        };
        ...
        let mut locked_flags = StakeFlags::empty();
        locked_flags.set(StakeFlags::PERMANENTLY_LOCKED);

        let mut stake_account = AccountSharedData::new_data_with_space(
            stake_lamports,
            &StakeStateV2::Stake(meta, stake, locked_flags),
            StakeStateV2::size_of(),
            &id(),
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            StakeError::PermanentlyLocked.into(),
        );
    }

    #[test]
    fn test_merge_permanently_locked_rejected() {
        let transaction_accounts = vec![(
            epoch_schedule::id(),
            create_account_shared_data_for_test(&EpochSchedule::default()),
        )];
        ...
        let locked_state: StakeStateV2 = StakeStateV2::Stake(meta, stake, locked_flags);
        let unlocked_state: StakeStateV2 = StakeStateV2::Stake(meta, stake, StakeFlags::empty());

        let clock = Clock::default();
        let stake_history = StakeHistory::default();
        let lamports = rent_exempt_reserve + 5_000_000;

        assert_eq!(
            MergeKind::get_if_mergeable(&invoke_context, &locked_state, lamports, &clock, &stake_history)
                .unwrap_err(),
            StakeError::PermanentlyLocked.into(),
        );

        assert!(MergeKind::get_if_mergeable(
            &invoke_context, &unlocked_state, lamports, &clock, &stake_history
        ).is_ok());
    }

    #[test]
    fn test_move_stake_permanently_locked_rejected() {
        let transaction_accounts = vec![(
            epoch_schedule::id(),
            create_account_shared_data_for_test(&EpochSchedule::default()),
        )];
        ...
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(StakeStateV2::size_of());
        let meta = Meta { rent_exempt_reserve, ..Meta::auto(&authority_pubkey) };
        let stake = Stake {
            delegation: Delegation {
                stake: 2_000_000, activation_epoch: 0, deactivation_epoch: u64::MAX,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        ...
        let locked_state = StakeStateV2::Stake(meta, stake, locked_flags);
        if let StakeStateV2::Stake(_, _, flags) = &locked_state {
            assert!(flags.contains(StakeFlags::PERMANENTLY_LOCKED));
        }

        let clock = Clock::default();
        let stake_history = StakeHistory::default();
        let lamports = rent_exempt_reserve + 2_000_000;
        assert_eq!(
            MergeKind::get_if_mergeable(&invoke_context, &locked_state, lamports, &clock, &stake_history)
                .unwrap_err(),
            StakeError::PermanentlyLocked.into(),
    ...
    #[test]
    fn test_move_lamports_permanently_locked_rejected() {
        let transaction_accounts = vec![(
            epoch_schedule::id(),
            create_account_shared_data_for_test(&EpochSchedule::default()),
        )];
        ...
        let stake = Stake {
            delegation: Delegation {
                stake: 3_000_000, activation_epoch: 0, deactivation_epoch: u64::MAX,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        ...
        let locked_state = StakeStateV2::Stake(meta, stake, locked_flags);
        let unlocked_state = StakeStateV2::Stake(meta, stake, StakeFlags::empty());

        let clock = Clock::default();
        let stake_history = StakeHistory::default();
        let lamports = rent_exempt_reserve + 3_000_000;
    ...
    #[test]
    fn test_deactivate_delinquent_permanently_locked_rejected() {
        let authority_pubkey = Pubkey::new_unique();
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(StakeStateV2::size_of());
        let meta = Meta { rent_exempt_reserve, ..Meta::auto(&authority_pubkey) };
        let stake = Stake {
            delegation: Delegation {
                stake: 4_000_000, activation_epoch: 0, deactivation_epoch: u64::MAX,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        ...
        if let StakeStateV2::Stake(_, _, flags) = state {
            assert!(flags.contains(StakeFlags::PERMANENTLY_LOCKED));
        }
    }

    // ── ProjectDawn permanent_lock / passive_lock / early_unlock tests ──

    #[test]
    fn test_permanent_lock_sets_flag() {
        let staker_pubkey = Pubkey::new_unique();
        ...
        let meta = Meta {
            rent_exempt_reserve,
            authorized: Authorized { staker: staker_pubkey, withdrawer: withdrawer_pubkey },
            ..Meta::default()
        };
        let stake = Stake {
            delegation: Delegation {
                voter_pubkey, stake: 1_000_000, activation_epoch: 0, deactivation_epoch: u64::MAX,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        ...
        let stake_account = AccountSharedData::new_data_with_space(
            stake_lamports, &StakeStateV2::Stake(meta, stake, StakeFlags::empty()),
            StakeStateV2::size_of(), &id(),
        ).unwrap();

        let state: StakeStateV2 = stake_account.state().unwrap();
        if let StakeStateV2::Stake(m, s, flags) = state {
            assert!(!flags.contains(StakeFlags::PERMANENTLY_LOCKED));
            let signers = HashSet::<Pubkey>::from_iter([staker_pubkey, withdrawer_pubkey]);
            assert!(m.authorized.check(&signers, StakeAuthorize::Staker).is_ok());
            assert!(m.authorized.check(&signers, StakeAuthorize::Withdrawer).is_ok());
            assert_eq!(s.delegation.deactivation_epoch, u64::MAX);
            assert_ne!(s.delegation.voter_pubkey, Pubkey::default());

            let mut new_flags = flags;
            new_flags.set(StakeFlags::PERMANENTLY_LOCKED);
            assert!(new_flags.contains(StakeFlags::PERMANENTLY_LOCKED));
        }
    }

    #[test]
    fn test_permanent_lock_requires_dual_signature() {
        let staker_pubkey = Pubkey::new_unique();
        let withdrawer_pubkey = Pubkey::new_unique();
        let meta = Meta {
            authorized: Authorized { staker: staker_pubkey, withdrawer: withdrawer_pubkey },
            ..Meta::auto(&staker_pubkey)
        };

        let staker_only = HashSet::<Pubkey>::from_iter([staker_pubkey]);
        assert!(meta.authorized.check(&staker_only, StakeAuthorize::Staker).is_ok());
        assert_eq!(
            meta.authorized.check(&staker_only, StakeAuthorize::Withdrawer).unwrap_err(),
            InstructionError::MissingRequiredSignature,
        );

        let both = HashSet::<Pubkey>::from_iter([staker_pubkey, withdrawer_pubkey]);
        assert!(meta.authorized.check(&both, StakeAuthorize::Staker).is_ok());
        assert!(meta.authorized.check(&both, StakeAuthorize::Withdrawer).is_ok());
    }

    #[test]
    fn test_permanent_lock_rejects_deactivated() {
        let voter_pubkey = Pubkey::new_unique();
        let stake = Stake {
            delegation: Delegation {
                voter_pubkey, stake: 1_000_000, activation_epoch: 0, deactivation_epoch: 100,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        assert_ne!(stake.delegation.deactivation_epoch, u64::MAX);
    }

    #[test]
    fn test_permanent_lock_rejects_already_locked() {
        let mut locked_flags = StakeFlags::empty();
        locked_flags.set(StakeFlags::PERMANENTLY_LOCKED);
        assert!(locked_flags.contains(StakeFlags::PERMANENTLY_LOCKED));
    }

    #[test]
    fn test_permanent_lock_rejects_passive() {
        let passive_stake = Stake {
            delegation: Delegation {
                voter_pubkey: Pubkey::default(), stake: 1_000_000,
                activation_epoch: 0, deactivation_epoch: u64::MAX,
                ..Delegation::default()
            },
            ..Stake::default()
        };
        assert_eq!(passive_stake.delegation.voter_pubkey, Pubkey::default());
    }

    #[test]
    fn test_passive_lock_sets_tier_and_lockup() {
        let tier: u8 = 3;
        let lockup_epochs: [u64; 6] = [0, 0, 14, 36, 72, 144];
        let current_epoch: u64 = 100;
        let mut stake_flags = StakeFlags::empty();
        stake_flags.set_passive_tier(tier);
        assert_eq!(stake_flags.passive_tier(), 3);
        assert_eq!(current_epoch.saturating_add(lockup_epochs[tier as usize]), 136);
    }

    #[test]
    fn test_passive_lock_rejects_invalid_tier() {
        assert!(0 < 1);
        assert!(6 > 5);
        for tier in 1..=5u8 {
    ...
    #[test]
    fn test_early_unlock_penalty_and_treasury_credit() {
        let tier_rate_bp: [u64; 6] = [0, 25, 50, 100, 150, 250];
        let stake_amount: u64 = 10_000_000_000;
        let expected_penalties: [u64; 6] = [0, 125_000_000, 250_000_000, 500_000_000, 750_000_000, 1_250_000_000];

        for tier in 1..=5usize {
            let rate_bp = tier_rate_bp[tier];
            let penalty = ((stake_amount as u128) * (rate_bp as u128) * 5 / 10_000) as u64;
            assert_eq!(penalty, expected_penalties[tier]);
        }

        // Verify u128 fix prevents overflow
        let large_stake: u64 = u64::MAX;
        let old_penalty = large_stake.saturating_mul(250).saturating_mul(5) / 10_000;
        let new_penalty = ((large_stake as u128) * 250 * 5 / 10_000) as u64;
        assert_ne!(old_penalty, new_penalty);

        // Conservation of lamports
        let rent = Rent::default();
        let rent_exempt_reserve = rent.minimum_balance(StakeStateV2::size_of());
        let initial_lamports = rent_exempt_reserve + stake_amount;
        let penalty = ((stake_amount as u128) * 100 * 5 / 10_000) as u64;
        let treasury_initial: u64 = 1_000_000;
        assert_eq!(initial_lamports + treasury_initial, (initial_lamports - penalty) + (treasury_initial + penalty));
    }

    // ── End ProjectDawn tests ──
