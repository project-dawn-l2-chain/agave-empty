...
impl Bank {
    ...
    pub(super) fn distribute_transaction_fee_details(&self) {
        // ProjectDawn L2: collector_id is the sequencer (sole block producer) in L2 mode.
        let fee_details = self.collector_fee_details.read().unwrap();
        let total_fees = fee_details.total();
        if total_fees == 0 {
            // nothing to distribute, exit early
            return;
        }

        if !self.projectdawn_enabled {
            // Stock Solana: 2-way split (burn + validator leader)
        ...
        // ProjectDawn: 4-way fee split (burn, validator, treasury, developer)
        let split = self.projectdawn_config.split_fees(total_fees, self.epoch());

        // 1. Burn
        self.capitalization.fetch_sub(split.burn, Relaxed);

        // 2. Validator (slot leader) — reuse existing deposit logic
        if split.validator > 0 {
            let mut burn_remainder = 0u64;
            self.deposit_or_burn_fee(split.validator, &mut burn_remainder);
            if burn_remainder > 0 {
                self.capitalization.fetch_sub(burn_remainder, Relaxed);
            }
        }
        ...
        if split.treasury > 0 {
            let mut burn_remainder = 0u64;
            match self.deposit_fees(&self.projectdawn_config.treasury_pubkey, split.treasury) {
                Ok(post_balance) => {
                    self.rewards.write().unwrap().push((
                        self.projectdawn_config.treasury_pubkey,
                        RewardInfo {
                            reward_type: RewardType::Fee,
                            lamports: split.treasury as i64,
                            post_balance,
                            commission: None,
                        },
                    ));
                ...
                Err(err) => {
                    debug!(
                        "Burned {} lamport treasury fee due to {}",
                        split.treasury, err
                    );
                    burn_remainder = split.treasury;
                }
            }
            if burn_remainder > 0 {
                self.capitalization.fetch_sub(burn_remainder, Relaxed);
        ...
        // 4. Developer rewards — distribute proportionally to program deployers
        if split.developer > 0 {
            self.distribute_developer_fees(split.developer);
        }
    }
    ...
    /// Distribute developer fee rewards proportionally to program deployers.
    ///
    /// Reads the per-program fee attribution map accumulated during transaction
    /// processing, then distributes `total_developer_fees` proportionally to
    /// each program's upgrade authority. Programs without an upgrade authority
    /// (immutable programs) have their share sent to treasury.
    fn distribute_developer_fees(&self, total_developer_fees: u64) {
        let attribution = self.program_fee_attribution.read().unwrap();
        if attribution.is_empty() {
            // No program attribution data; send everything to treasury
            let mut burn_remainder = 0u64;
            match self.deposit_fees(&self.projectdawn_config.treasury_pubkey, total_developer_fees) {
                Ok(_) => {}
                Err(_) => { burn_remainder = total_developer_fees; }
            }
            if burn_remainder > 0 {
                self.capitalization.fetch_sub(burn_remainder, Relaxed);
            }
        ...
        let total_attributed: u64 = attribution.values().sum();
        if total_attributed == 0 {
            self.capitalization.fetch_sub(total_developer_fees, Relaxed);
            return;
        }

        let mut distributed = 0u64;
        let entries: Vec<_> = attribution.iter().map(|(k, v)| (*k, *v)).collect();
        drop(attribution); // release lock before account lookups

        for (program_id, program_fees) in &entries {
            let share = total_developer_fees
                .checked_mul(*program_fees)
                .unwrap_or(0)
                / total_attributed;
            if share == 0 {
                continue;
            ...
            // Look up the program's upgrade authority
            let recipient = self.get_program_upgrade_authority(program_id)
                .unwrap_or(self.projectdawn_config.treasury_pubkey);

            match self.deposit_fees(&recipient, share) {
                Ok(_) => {
                    distributed += share;
                }
                Err(_) => {
                    // If deposit fails, try treasury; if that fails, burn
                    match self.deposit_fees(&self.projectdawn_config.treasury_pubkey, share) {
                        Ok(_) => { distributed += share; }
                        Err(_) => { /* will be burned below */ }
                    }
                }
            }
        }

        // Burn any undistributed remainder
        let remainder = total_developer_fees.saturating_sub(distributed);
        if remainder > 0 {
            self.capitalization.fetch_sub(remainder, Relaxed);
        }
    }

    /// Look up the upgrade authority for an upgradeable BPF program.
    ///
    /// Returns `Some(authority)` if the program is upgradeable and has an authority,
    /// or `None` if the program is immutable or not found.
    fn get_program_upgrade_authority(&self, program_id: &Pubkey) -> Option<Pubkey> {
        use solana_sdk::bpf_loader_upgradeable::{self, UpgradeableLoaderState};

        let account = self.get_account_with_fixed_root_no_cache(program_id)?;
        if account.owner() != &bpf_loader_upgradeable::id() {
            return None;
        }

        // The program account contains a pointer to the programdata account
        let program_state: UpgradeableLoaderState = bincode::deserialize(account.data()).ok()?;
        if let UpgradeableLoaderState::Program { programdata_address } = program_state {
            let programdata_account = self.get_account_with_fixed_root_no_cache(&programdata_address)?;
            let programdata_state: UpgradeableLoaderState =
                bincode::deserialize(programdata_account.data()).ok()?;
            if let UpgradeableLoaderState::ProgramData { upgrade_authority_address, .. } = programdata_state {
                return upgrade_authority_address;
            }
        }
        None
    }
