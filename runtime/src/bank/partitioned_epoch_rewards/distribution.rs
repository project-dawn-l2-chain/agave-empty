...
impl Bank {
    ...
    fn build_updated_stake_reward(
        ...
        // ProjectDawn: For permanently locked accounts the reward lamports
        // include a 20% bonus that is NOT compounded into delegation.stake.
        // The compounded portion equals the increase in delegation.stake;
        // the remainder is excess (non-compounding) lamports.
        let delegation_increase = partitioned_stake_reward
            .stake
            .delegation
            .stake
            .saturating_sub(stake.delegation.stake);
        assert!(
            partitioned_stake_reward.stake_reward_info.lamports as u64 >= delegation_increase,
            "reward lamports must cover at least the delegation stake increase"
        );
        assert_eq!(
            stake.delegation.stake.saturating_add(delegation_increase),
