...
impl Bank {
    ...
    pub(in crate::bank) fn begin_partitioned_rewards(
        ...
        // Include the permanent-lock bonus in the total rewards so the
        // EpochRewards sysvar distribution assertion holds.
        let total_point_value = PointValue {
            rewards: point_value.rewards.saturating_add(permanent_lock_bonus_lamports),
    ...
    fn calculate_rewards_and_distribute_vote_rewards(
        ...
        // The permanent_lock_bonus_lamports is additional minted inflation
        // for the 120% permanent-lock reward multiplier, funded outside the
        // base reward pool.
        let base_stake_rewards = total_stake_rewards_lamports
            .saturating_sub(permanent_lock_bonus_lamports);
        assert!(point_value.rewards >= validator_rewards_paid + base_stake_rewards);

        info!(
            "distributed vote rewards: {} out of {}, remaining {} (permanent lock bonus: {})",
        ...
        // Add vote rewards + permanent lock bonus to capitalization.
        // The bonus is new inflation beyond the base reward pool.
        self.capitalization
            .fetch_add(validator_rewards_paid.saturating_add(permanent_lock_bonus_lamports), Relaxed);
    ...
    pub(super) fn calculate_rewards_for_partitioning(
        ...
        PartitionedRewardsCalculation {
            ...
            stake_rewards_by_partition: StakeRewardCalculationPartitioned {
                ...
                permanent_lock_bonus_lamports: stake_rewards.permanent_lock_bonus_lamports,
            },
            old_vote_balance_and_staked,
            validator_rate,
            foundation_rate,
    ...
    fn calculate_stake_vote_rewards(
        ...
        let permanent_lock_bonus = AtomicU64::default();
        let _projectdawn_enabled = self.projectdawn_enabled;
        let projectdawn_config = self.projectdawn_config.clone();
        let (stake_rewards, measure_stake_rewards_us) = measure_us!(thread_pool.install(|| {
            stake_delegations
                ...
                .filter_map(|(stake_pubkey, stake_account)| {
                    ...
                    // ProjectDawn: Handle passive stakes (sentinel voter_pubkey)
                    if vote_pubkey == Pubkey::default() {
                        if let StakeStateV2::Stake(_meta, stake, stake_flags) = &stake_state {
                            let tier = stake_flags.passive_tier();
                            if tier > 0 && tier <= 5 {
                                let rate_bp = projectdawn_config.passive_tier_rate_bp(tier);
                                let stakers_reward = stake.delegation.stake
                                    .saturating_mul(rate_bp)
                                    / (crate::projectdawn_config::BASIS_POINTS_100_PCT
                                        * crate::projectdawn_config::EPOCHS_PER_YEAR);

                                if stakers_reward > 0 {
                                    if stake_account.checked_add_lamports(stakers_reward).is_ok() {
                    ...
                    if let Ok((stakers_reward, voters_reward)) = redeemed {
                        // ProjectDawn: 120% reward multiplier for permanently
                        // locked stakes. The bonus 20% is extra minted inflation,
                        // added as excess lamports (not compounded into delegation).
                        let bonus = if let StakeStateV2::Stake(_, _, flags) = &stake_state {
                            if flags.contains(StakeFlags::PERMANENTLY_LOCKED) {
                                stakers_reward.saturating_mul(20) / 100
                            } else {
                                0
                            }
                        } else {
                        ...
                        let total_staker_reward = stakers_reward.saturating_add(bonus);
                        if bonus > 0 {
                            let _ = stake_account.checked_add_lamports(bonus);
                            permanent_lock_bonus.fetch_add(bonus, Relaxed);
                        }

                        debug!(
                            "calculated reward: {} {} {} {}",
                            stake_pubkey, pre_lamport, stake_account.lamports(), total_staker_reward
                        );
                        ...
                        let post_balance = stake_account.lamports();
                        total_stake_rewards.fetch_add(total_staker_reward, Relaxed);
                        ...
                        return Some(PartitionedStakeReward {
                            ...
                            stake_reward_info: RewardInfo {
                                reward_type: RewardType::Staking,
                                lamports: i64::try_from(total_staker_reward).unwrap(),
                                post_balance,
        ...
        (
            ...
            StakeRewardCalculation {
                ...
                permanent_lock_bonus_lamports: permanent_lock_bonus.load(Relaxed),
            },
        )
    }
