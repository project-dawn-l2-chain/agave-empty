...
use {
    crate::{
        ...
        projectdawn_config::ProjectDawnConfig,
...
pub struct BankFieldsToDeserialize {
    ...
    // ProjectDawn L2: In L2 mode, the validator (collector_id) operates as the sequencer.
    ...
    /// ProjectDawn config (populated from ExtraFields, None if not present in snapshot)
    pub(crate) projectdawn_config: Option<ProjectDawnConfig>,
}
...
pub struct BankFieldsToSerialize {
    ...
    /// ProjectDawn: custom tokenomics config for snapshot serialization
    pub projectdawn_config: ProjectDawnConfig,
}
...
impl PartialEq for Bank {
    fn eq(&self, other: &Self) -> bool {
        ...
        let Self {
            ...
            block_id,
            bank_hash_stats: _,
            projectdawn_config: _,
            projectdawn_enabled: _,
            program_fee_attribution: _,
...
impl BankFieldsToSerialize {
    ...
    pub fn default_for_tests() -> Self {
        Self {
            ...
            projectdawn_config: ProjectDawnConfig::default(),
        }
    }
}
...
pub struct Bank {
    ...
    /// ProjectDawn: Configuration for custom tokenomics (fee split, staking APY, etc.)
    pub projectdawn_config: Arc<ProjectDawnConfig>,

    /// ProjectDawn: Whether ProjectDawn features are enabled (set via feature gate)
    pub projectdawn_enabled: bool,

    /// ProjectDawn: Per-program fee attribution for developer rewards
    program_fee_attribution: RwLock<HashMap<Pubkey, u64>>,
}
...
impl Bank {
    fn default_with_accounts(accounts: Accounts) -> Self {
        let mut bank = Self {
            ...
            projectdawn_config: Arc::new(ProjectDawnConfig::default()),
            projectdawn_enabled: false,
            program_fee_attribution: RwLock::new(HashMap::new()),
        };
    ...
    fn _new_from_parent(
        ...
        let mut new = Self {
            ...
            block_id: RwLock::new(None),
            bank_hash_stats: AtomicBankHashStats::default(),
            projectdawn_config: parent.projectdawn_config.clone(),
            projectdawn_enabled: parent.projectdawn_enabled,
            program_fee_attribution: RwLock::new(HashMap::new()),
        };
    ...
    pub(crate) fn new_from_fields(
        ...
        let mut bank = Self {
            ...
            projectdawn_config: Arc::new(fields.projectdawn_config.unwrap_or_default()),
            projectdawn_enabled: false,
            program_fee_attribution: RwLock::new(HashMap::new()),
        };
    ...
    pub(crate) fn get_fields_to_serialize(&self) -> BankFieldsToSerialize {
        ...
        BankFieldsToSerialize {
            ...
            projectdawn_config: (*self.projectdawn_config).clone(),
        }
    }
    ...
    fn calculate_previous_epoch_inflation_rewards(
        ...
        let validator_rewards = if self.projectdawn_enabled {
            // ProjectDawn: Flat 5% APY of staked supply (integer-only arithmetic)
            let total_staked = self
                ...
                .values()
                .sum::<u64>();
            self.projectdawn_config.epoch_staking_reward(total_staked)
        } else {
            // Stock Solana: inflation-based rewards on total capitalization
            (validator_rate * prev_epoch_capitalization as f64 * prev_epoch_duration_in_years)
                as u64
        };
    ...
    fn filter_program_errors_and_collect_fee_details(
        ...
        for (i, processing_result) in processing_results.iter().enumerate() {
            if let Ok(processed_tx) = processing_result {
                let fee = processed_tx.fee_details();
                accumulated_fee_details.accumulate(&fee);

                // ProjectDawn: Track per-program fee attribution for developer rewards.
                // Only attribute fees from successfully executed transactions.
                if self.projectdawn_enabled
                    && processed_tx
                        .execution_details()
                        .map_or(false, |d| d.status.is_ok())
                {
                    if let Some(tx) = sanitized_txs.get(i) {
                        let total_tx_fee = fee.total_fee();
                        if total_tx_fee > 0 {
                            let program_ids: Vec<solana_pubkey::Pubkey> = tx
                            ...
                            if !program_ids.is_empty() {
                                let per_program = total_tx_fee / program_ids.len() as u64;
                                let mut attribution =
                                    self.program_fee_attribution.write().unwrap();
                                for program_id in &program_ids {
                                    let entry = attribution.entry(*program_id).or_insert(0);
                                    *entry = entry.saturating_add(per_program);
                                }
                            }
                        }
                    }
    ...
    pub fn commit_transactions(
        ...
        self.filter_program_errors_and_collect_fee_details(sanitized_txs, &processing_results);
    ...
    fn apply_feature_activations(
        ...
        // ProjectDawn: Inject custom feature ID into the inactive set so it can be
        // activated via `solana feature activate` like any standard feature.
        {
            use crate::projectdawn_config::PROJECTDAWN_FEATURE_ID;
            let mut inactive = self.feature_set.inactive.clone();
            inactive.insert(PROJECTDAWN_FEATURE_ID);
            self.feature_set = Arc::new(solana_feature_set::FeatureSet {
                active: self.feature_set.active.clone(),
                inactive,
            });
        }
        let (feature_set, new_feature_activations) =
            self.compute_active_feature_set(allow_new_activations);
        // ProjectDawn: Check if our custom feature is now active
        self.projectdawn_enabled = feature_set.is_active(&crate::projectdawn_config::PROJECTDAWN_FEATURE_ID);
        self.feature_set = Arc::new(feature_set);
